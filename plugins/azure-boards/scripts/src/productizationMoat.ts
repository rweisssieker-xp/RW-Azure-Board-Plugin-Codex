...
import { finding, normalizeItems, objectFrom, recordArray, report, stringFrom, numberFrom } from "./requirementsWorkbench.js";
...
export function connectorReadinessAudit(connectors: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; readiness: Array<Record<string, unknown>> } {
  ...
  const findings = readiness
    ...
    .map((row) => finding(Number(row.id), String(row.connector), 100 - Number(row.score), row.missing as string[], String(row.recommendation)));
...
export function evidenceIngestionPipeline(records: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; pipeline: Array<Record<string, unknown>>; controls: Record<string, unknown> } {
...
export function securityPrivacyReview(config: Record<string, unknown> = {}, dataFlows: Record<string, unknown>[] = []): Report & { writePerformed: false; controls: Array<Record<string, unknown>> } {
  const controls = [
...
export function marketplaceSubmissionReadiness(submission: Record<string, unknown> = {}, assets: Record<string, unknown>[] = []): Report & { writePerformed: false; checklist: Array<Record<string, unknown>> } {
  ...
  checklist.push(
    { id: checklist.length + 1, item: "tool annotations", status: Object.keys(submittedTools).length ? "ready" : "missing", recommendation: "Every runtime tool should have matching annotations and justifications." },
    { id: checklist.length + 1, item: "extension package", status: submission.extensionPackage ? "ready" : "partial", recommendation: "Validate Azure DevOps extension manifest, screenshots, and backend URL settings." }
  );
...
export function orgRolloutReadiness(orgConfig: Record<string, unknown> = {}, teams: Record<string, unknown>[] = []): Report & { writePerformed: false; rollout: Array<Record<string, unknown>> } {
  const baseControls = [
...
export function licensePackagingAdvisor(usage: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; packages: Array<Record<string, unknown>> } {
...
export function customerValueCaseBuilder(workItems: InputItem[], evidence: Record<string, unknown>[] = []): Report & { writePerformed: false; valueCases: Array<Record<string, unknown>> } {
...
export function proprietarySignalCatalog(workItems: InputItem[], feedback: Record<string, unknown>[] = [], evidence: Record<string, unknown>[] = []): Report & { writePerformed: false; signals: Array<Record<string, unknown>> } {
...
export function autonomousFollowupScheduler(workItems: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; followups: Array<Record<string, unknown>> } {
...
export function adoptionExperimentDesigner(usage: Record<string, unknown>[] = [], teams: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; experiments: Array<Record<string, unknown>> } {
...
function normalizeConnector(connector: Record<string, unknown>, index: number): { name: string; category: string; configured: boolean; hasScopes: boolean; hasOwner: boolean; hasHealthCheck: boolean } {
  return {
...
function missingConnectorControls(connector?: { hasScopes: boolean; hasOwner: boolean; hasHealthCheck: boolean }): string[] {
  if (!connector) return ["configuration"];
  return [
...
function classifyRecord(record: Record<string, unknown>, text: string): string {
  if (SENSITIVE_WORDS.test(`${JSON.stringify(record)} ${text}`)) return "sensitive";
  if (/public|marketing|sample/i.test(`${record.classification || ""} ${text}`)) return "public";
  return "internal";
}

function inferType(record: Record<string, unknown>, text: string): string {
  const fileName = `${stringFrom(record.name)} ${stringFrom(record.fileName)}`.toLowerCase();
  if (/\.pdf\b/.test(fileName)) return "pdf";
  if (/\.docx?\b/.test(fileName)) return "document";
  if (/\.xlsx?|\.csv\b/.test(fileName)) return "tabular";
  if (/image|png|jpg|jpeg|mockup/i.test(`${record.type || ""} ${fileName} ${text}`)) return "image-metadata";
  return "text";
}

function extractedSignals(text: string): string[] {
  return [
...
function evidenceFor(id: number, evidence: Record<string, unknown>[]): Record<string, unknown>[] {
  return evidence.filter((entry) => (numberFrom(entry.workItemId) ?? numberFrom(entry.id)) === id);
}

function benefitEstimate(raw: Record<string, unknown>, evidence: Record<string, unknown>[]): number {
...
function valueField(raw: Record<string, unknown>): number {
  const fields = objectFrom(raw.fields);
...
function signal(name: string, count: number, use: string): Record<string, unknown> {
