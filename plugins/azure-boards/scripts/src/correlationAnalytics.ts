...
type DeliveryKind = "pullRequest" | "build" | "pipeline" | "release";
type DeliveryHealth = "healthy" | "warning" | "failed" | "unknown";

export interface DeliverySystemEvidence {
...
export interface DeliverySystemCorrelationOptions {
  asOfDate?: string;
  maxFindings?: number;
  staleDays?: number;
  includeHealthyItems?: boolean;
}
...
interface ItemCorrelation {
  ...
  pullRequests: NormalizedEvidence[];
  builds: NormalizedEvidence[];
  releases: NormalizedEvidence[];
  pipelines: NormalizedEvidence[];
  failed: NormalizedEvidence[];
  warnings: NormalizedEvidence[];
}

const CLOSED_STATES = new Set(["closed", "done", "removed", "resolved", "completed"]);
const FIXED_GENERATED_AT = "1970-01-01T00:00:00.000Z";

export function deliverySystemCorrelation(
  workItems: Array<WorkItemSummary | WorkItem>,
  evidence: DeliverySystemEvidence = {},
  options: DeliverySystemCorrelationOptions = {}
): Report {
  const items = workItems.map(normalizeWorkItem).filter((item): item is NormalizedWorkItem => Boolean(item));
...
function normalizeWorkItem(item: WorkItemSummary | WorkItem): NormalizedWorkItem | undefined {
  const id = numberValue((item as WorkItemSummary).id);
  if (id === undefined) return undefined;
  const summary = item as WorkItemSummary;
  const fields = isRecord((item as WorkItem).fields) ? (item as WorkItem).fields : {};
  return {
    id,
...
function normalizePullRequests(input: unknown): NormalizedEvidence[] {
...
function normalizeBuilds(input: unknown): NormalizedEvidence[] {
  return arrayFrom(input).map((entry) => {
...
function normalizePipelines(input: unknown): NormalizedEvidence[] {
  return arrayFrom(input).map((entry) => {
...
function normalizeReleases(input: unknown): NormalizedEvidence[] {
  return arrayFrom(input).map((entry) => {
...
function correlate(items: NormalizedWorkItem[], evidence: NormalizedEvidence[]): ItemCorrelation[] {
  const knownIds = new Set(items.map((item) => item.id));
  const byId = new Map<number, NormalizedEvidence[]>();
...
function globalHealthFindings(evidence: NormalizedEvidence[]): Finding[] {
...
function itemFindings(entry: ItemCorrelation, asOf: Date, options: DeliverySystemCorrelationOptions): Finding[] {
  const findings: Finding[] = [];
...
function mentionedWorkItemIds(text: string, knownIds: Set<number>): number[] {
  ...
  for (const match of text.matchAll(prefixed)) {
    ...
    if (id !== undefined && knownIds.has(id)) found.add(id);
  }
  for (const id of knownIds) {
    const escaped = String(id).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const branchOrToken = new RegExp(`(?:^|[^0-9a-z])${escaped}(?:[^0-9a-z]|$)`, "i");
    if (branchOrToken.test(text)) found.add(id);
  }
  return [...found];
}

function referenceDate(items: NormalizedWorkItem[], evidence: NormalizedEvidence[], override?: string): Date {
...
function arrayFrom(input: unknown): Record<string, unknown>[] {
...
function collectNestedStrings(input: unknown, keys: string[], limit = 20): string[] {
  const found: string[] = [];
...
function isClosed(item: NormalizedWorkItem): boolean {
  return CLOSED_STATES.has(item.state.toLowerCase());
}

function healthForPullRequest(status: string, result: string): DeliveryHealth {
  const text = `${status} ${result}`.toLowerCase();
  if (failedWord(text) || text.includes("conflict") || text.includes("rejected")) return "failed";
  if (warningWord(text) || text.includes("active") || text.includes("notset") || text.includes("queued")) return "warning";
  if (text.includes("completed") || text.includes("succeeded") || text.includes("approved")) return "healthy";
  return "unknown";
}

function healthForRun(status: string, result: string): DeliveryHealth {
  const text = `${status} ${result}`.toLowerCase();
  if (failedWord(text)) return "failed";
  if (warningWord(text) || text.includes("inprogress") || text.includes("in progress") || text.includes("queued") || text.includes("notstarted")) return "warning";
  if (text.includes("succeeded") || text.includes("completed") || text.includes("partiallysuccessful")) return "healthy";
  return "unknown";
}

function failedWord(value: string): boolean {
...
function warningWord(value: string): boolean {
  const text = value.toLowerCase();
...
function label(kind: DeliveryKind): string {
