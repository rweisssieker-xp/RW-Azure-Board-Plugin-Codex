...
export function autonomousBoardAuditor(items: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; auditRows: Array<Record<string, unknown>> } {
...
export function requirementRewriteStudio(items: InputItem[]): Report & { writePerformed: false; rewrites: Array<Record<string, unknown>> } {
...
export function decisionMeetingCopilot(items: InputItem[], evidence: Record<string, unknown>[] = []): Report & { writePerformed: false; agenda: Array<Record<string, unknown>>; minutesDraft: string } {
...
export function cleanupCampaignManager(items: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; campaign: Record<string, unknown>; actions: Array<Record<string, unknown>> } {
...
export function financialBacklogLedger(items: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; ledger: Array<Record<string, unknown>> } {
...
export function requirementConfidenceScore(items: InputItem[]): Report & { writePerformed: false; scores: Array<Record<string, unknown>> } {
  ...
  const findings = scores.map((row) => finding(Number(row.id), String(row.title), Number(row.confidenceScore), row.signals as string[], String(row.status) === "ready" ? "Requirement is decision-ready; keep evidence current." : "Clarify evidence, owner, value, and acceptance criteria before investment."));
...
export function dependencyBlockerGraph(items: InputItem[], comments: Record<string, unknown>[] = []): Report & { writePerformed: false; graph: { nodes: Array<Record<string, unknown>>; edges: Array<Record<string, unknown>> } } {
  const normalized = normalizeItems(items);
  ...
  for (const item of normalized) {
    ...
    if (RISK_WORDS.test(textFor(item))) edges.push({ from: `risk:${item.id}`, to: `wi:${item.id}`, relation: "risk-signal" });
    for (const link of item.links) edges.push({ from: `wi:${item.id}`, to: `link:${item.id}:${link}`, relation: "linked-artifact" });
  }
  for (const comment of comments) {
    const id = numberFrom(comment.workItemId) ?? numberFrom(comment.id);
    if (id && RISK_WORDS.test(evidenceText(comment))) edges.push({ from: `comment:${id}`, to: `wi:${id}`, relation: "comment-blocker" });
  }
...
export function processOwnerControlTower(items: InputItem[], policy: Record<string, unknown> = {}): Report & { writePerformed: false; teams: Array<Record<string, unknown>> } {
  const normalized = normalizeItems(items);
...
export function migrationCutoverReadiness(items: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; readiness: Record<string, unknown> } {
  const normalized = normalizeItems(items);
...
export function aiExceptionRegister(items: InputItem[], evidence: Record<string, unknown>[] = []): Report & { writePerformed: false; exceptions: Array<Record<string, unknown>> } {
  const exceptions = normalizeItems(items).flatMap((item) => {
    const texts = [textFor(item), ...evidenceFor(item, evidence).map(evidenceText)];
    if (!texts.some((text) => DECISION_WORDS.test(text) && /exception|waiver|defer|accepted/i.test(text))) return [];
    return [{
      id: item.id,
      title: item.title,
      owner: item.assignedTo || "unassigned",
...
export function benefitRealizationFollowup(items: InputItem[], options: Record<string, unknown> = {}): Report & { writePerformed: false; followups: Array<Record<string, unknown>> } {
...
function confidenceSignals(item: NormalizedItem): string[] {
...
function campaignMatch(item: NormalizedItem, target: string): boolean {
  if (target.includes("low-wsjf") || target.includes("low-value")) return !isClosed(item) && expectedBenefit(item) < 30_000;
  if (target.includes("missing-description")) return REQUIREMENT_TYPES.has(item.type.toLowerCase()) && item.description.length < 80;
  if (target.includes("orphan") || target.includes("task")) return item.type.toLowerCase() === "task" && !item.parentId;
  if (target.includes("closed-evidence")) return isClosed(item) && !evidenceSignals(item).length;
  return !isClosed(item) && (daysSince(item.changedDate) > 90 || !item.assignedTo || item.description.length < 80);
}

function campaignRationale(item: NormalizedItem, target: string): string {
...
function decisionAsk(item: NormalizedItem): string {
  if (!item.assignedTo) return "Assign accountable owner or park the work.";
  if (RISK_WORDS.test(textFor(item))) return "Decide escalation, dependency owner, or risk acceptance.";
  if (confidenceScore(item) < 45) return "Decide whether to rework or close due to weak evidence.";
  if (expectedBenefit(item) < implementationCost(item, {})) return "Decide whether the business case justifies further spend.";
  return "Confirm priority, target date, and next delivery step.";
}

function scoreDecisionNeed(item: NormalizedItem): number {
  let score = 0;
  if (!item.assignedTo) score += 25;
  if (RISK_WORDS.test(textFor(item))) score += 30;
  if (confidenceScore(item) < 45) score += 25;
  if (daysSince(item.changedDate) > 60) score += 20;
  if (priority(item) <= 2) score += 10;
  return Math.min(100, score);
}

function cutoverSignals(item: NormalizedItem): string[] {
  const signals = [`priority ${priority(item)}`, `state ${item.state}`];
  if (ERP_CRITICAL_WORDS.test(textFor(item))) signals.push("ERP critical domain");
  if (!evidenceSignals(item).length) signals.push("missing evidence");
  if (RISK_WORDS.test(textFor(item))) signals.push("risk/blocker language");
  return signals;
}
...
function realizedBenefit(item: NormalizedItem): number {
  return firstNumber(item, REALIZED_FIELDS) ?? 0;
}

function implementationCost(item: NormalizedItem, options: Record<string, unknown>): number {
  const explicit = firstNumber(item, COST_FIELDS);
  if (explicit !== undefined) return explicit;
  const effort = firstNumber(item, EFFORT_FIELDS);
  if (effort !== undefined) return effort * positive(options.defaultStoryPointCost, 1_500);
...
function evidenceSignals(item: NormalizedItem): string[] {
  ...
  if (DECISION_WORDS.test(item.description)) signals.push("decision language");
  return signals;
}

function evidenceFor(item: NormalizedItem, evidence: Record<string, unknown>[]): Record<string, unknown>[] {
  return evidence.filter((entry) => (numberFrom(entry.workItemId) ?? numberFrom(entry.id)) === item.id);
}

function domainFor(item: NormalizedItem): string {
  const text = textFor(item);
  if (/finance|rechnung|invoice|closing/i.test(text)) return "Finance";
  if (/production|fertigung|warehouse|lager/i.test(text)) return "Operations";
  if (/regulatory|audit|gudid|udi|eudamed|compliance/i.test(text)) return "Regulatory";
  if (/integration|schnittstelle|api|ssis/i.test(text)) return "Integration";
  if (/customer|kunde|portal|crm/i.test(text)) return "Customer";
...
function mergeTags(tags: string[], tag: string): string {
