...
export interface DecisionCockpitReport extends Report {
  writePerformed: false;
  decisions: DecisionCockpitItem[];
}

export interface DecisionCockpitItem {
  id: number;
  title: string;
  state: string;
  decision: "accelerate" | "review" | "park" | "close";
  score: number;
  rationale: string[];
}

export interface EvidenceReviewReport extends Report {
  writePerformed: false;
...
export interface CioRiskViewReport extends Report {
  writePerformed: false;
  riskItems: CioRiskItem[];
}

export interface CioRiskItem {
  id: number;
  title: string;
  riskScore: number;
...
type DecisionInput = WorkItem | WorkItemSummary | Record<string, unknown>;
...
export function requirementDecisionCockpit(items: DecisionInput[], options: Record<string, unknown> = {}): DecisionCockpitReport {
...
export function evidenceFirstRequirementReview(items: DecisionInput[], evidence: Record<string, unknown>[] = []): EvidenceReviewReport {
...
export function cioRequirementRiskView(items: DecisionInput[], options: Record<string, unknown> = {}): CioRiskViewReport {
...
function decisionFor(item: NormalizedItem, minEvidenceScore: number): DecisionCockpitItem {
  ...
  let decision: DecisionCockpitItem["decision"] = "review";
  if (isTerminal(item.state)) decision = "close";
  else if (evidence < minEvidenceScore) decision = stale >= 120 ? "park" : "review";
  else if (score >= 75) decision = "accelerate";
  else if (score < 35 && stale >= 90) decision = "park";
  return { id: item.id, title: item.title, state: item.state, decision, score, rationale };
}

function evidenceReviewFor(item: NormalizedItem, suppliedEvidence: string[]): EvidenceReviewItem {
  const score = evidenceScore(item, suppliedEvidence);
  const missing: string[] = [];
  if (item.description.length < 160) missing.push("clear problem statement");
  if (item.acceptanceCriteria.length < 80) missing.push("acceptance criteria");
  if (!attachmentNames(item.raw).length && !suppliedEvidence.length) missing.push("attachment or supplied evidence");
  if (!VALUE_TERMS.some((term) => searchableText(item).includes(term))) missing.push("business value signal");
...
function cioRiskFor(item: NormalizedItem, staleDays: number): CioRiskItem {
...
function requirementItems(items: DecisionInput[]): NormalizedItem[] {
  return normalizeItems(items).filter((item) => REQUIREMENT_TYPES.has(item.type.toLowerCase()));
}

function normalizeItems(items: DecisionInput[]): NormalizedItem[] {
...
function evidenceScore(item: NormalizedItem, suppliedEvidence: string[]): number {
  let score = 0;
  ...
  if (EVIDENCE_FIELDS.some((field) => textFromFields(objectFrom(item.raw.fields), [field]).length >= 80)) score += 10;
  return clamp(score, 0, 100);
}

function evidenceMap(evidence: Record<string, unknown>[]): Map<number, string[]> {
...
function keywordScore(item: NormalizedItem, terms: string[], perHit: number): number {
  const text = searchableText(item);
  const hits = terms.filter((term) => text.includes(term)).length;
  return Math.min(40, hits * perHit);
}

function searchableText(item: NormalizedItem): string {
  return `${item.title} ${item.description} ${item.acceptanceCriteria} ${item.tags.join(" ")}`.toLowerCase();
}

function recommendationForDecision(decision: DecisionCockpitItem["decision"]): string {
...
function textFromFields(fields: Record<string, unknown>, names: string[]): string {
