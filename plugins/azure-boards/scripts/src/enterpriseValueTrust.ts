...
export function businessDigitalTwin(workItems: InputItem[], evidence: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; twin: Array<Record<string, unknown>> } {
...
export function externalEvidenceImport(records: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; importedEvidence: Array<Record<string, unknown>> } {
...
export function eventLogProcessMining(events: Record<string, unknown>[] = [], options: Record<string, unknown> = {}): Report & { writePerformed: false; processMap: Record<string, unknown> } {
  ...
  for (const [caseId, caseEvents] of Object.entries(cases)) {
    ...
    for (let index = 1; index < sorted.length; index += 1) {
      ...
      if (waitHours >= positive(options.bottleneckHours, 48)) bottlenecks.push({ caseId, from, to, waitHours: Math.round(waitHours) });
    }
  }
...
export function stakeholderInfluenceMap(workItems: InputItem[], stakeholders: Record<string, unknown>[] = []): Report & { writePerformed: false; map: { nodes: Array<Record<string, unknown>>; edges: Array<Record<string, unknown>> } } {
...
export function roiConfidenceWorkflow(workItems: InputItem[], financeEvidence: Record<string, unknown>[] = []): Report & { writePerformed: false; roi: Array<Record<string, unknown>> } {
...
export function enterpriseRiskHeatmap(workItems: InputItem[], signals: Record<string, unknown>[] = []): Report & { writePerformed: false; heatmap: Array<Record<string, unknown>> } {
...
export function policyStudio(workItems: InputItem[], policy: Record<string, unknown> = {}): Report & { writePerformed: false; policyDraft: Record<string, unknown>; simulation: Array<Record<string, unknown>> } {
  const policyDraft = {
  ...
  const findings = simulation.filter((row) => row.status === "would-fail").map((row) => finding(Number(row.id), String(row.title), 65, row.violations as string[], "Fix policy violations or add an exception before enforcing this policy."));
...
export function promptEvalSuite(prompts: Record<string, unknown>[] = [], cases: Record<string, unknown>[] = []): Report & { writePerformed: false; evals: Array<Record<string, unknown>> } {
...
export function modelRiskGovernance(config: Record<string, unknown> = {}, dataClasses: Record<string, unknown>[] = []): Report & { writePerformed: false; modelRisk: Array<Record<string, unknown>>; policy: Record<string, unknown> } {
...
export function adoptionCockpit(usage: Record<string, unknown>[] = [], workItems: InputItem[] = []): Report & { writePerformed: false; adoption: Array<Record<string, unknown>> } {
...
function valueScore(item: { raw: Record<string, unknown>; title: string; description: string; tags: string[] }): number {
  const fields = objectFrom(item.raw.fields);
...
function inferEvidenceType(record: Record<string, unknown>): string {
  const text = JSON.stringify(record).toLowerCase();
  if (/ticket|support/.test(text)) return "support";
  if (/incident|outage/.test(text)) return "incident";
  if (/revenue|cost|finance|amount/.test(text)) return "finance";
  if (/event|activity|case/.test(text)) return "event-log";
  return "generic";
}

function evidenceConfidence(record: Record<string, unknown>): number {
