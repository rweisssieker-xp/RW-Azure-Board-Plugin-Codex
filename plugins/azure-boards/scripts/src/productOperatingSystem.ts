...
export function createPersistentBaseline(
  name: string,
  items: WorkItemSummary[],
...
export function approvalQueue(
  ...
  recommendations: Record<string, unknown>[] = [],
  options: Record<string, unknown> = {}
): Report & { writePerformed: false; approvalRequired: true; queue: Array<Record<string, unknown>> } {
  const policy = objectFrom(options.policy);
  const sourceActions = (recommendations.length ? recommendations : actionPlan(items, policy, options).actions || []) as Record<string, unknown>[];
  ...
  const result = {
    ...report(
      "Approval Queue",
      queue.map((entry, index) => finding(numberFrom(entry.workItemId), stringFrom(entry.title), entry.risk === "high" ? 85 : 55, ["approval required", `${entry.risk} risk`], "Select, reject, or override before any apply workflow.")),
      `${queue.length} recommendation(s) prepared for approval review.`,
      { approvalsPending: queue.length, selectedByDefault: queue.filter((entry) => entry.selected).length }
    ),
...
export function approvalApplyPlan(
  queue: Record<string, unknown>[] = [],
  selection: Record<string, unknown> = {},
  options: Record<string, unknown> = {}
): Report & { writePerformed: false; approvalRequired: true; plan: Array<Record<string, unknown>>; auditEvents: Array<Record<string, unknown>> } {
  ...
  return {
    ...report(
      "Approval Apply Plan",
      plan.filter((entry) => entry.status === "needs-secondary-approval").map((entry) => finding(numberFrom(entry.workItemId), stringFrom(entry.title), 90, ["high risk", "secondary approval required"], "Get secondary approval or override rationale before applying.")),
      `${plan.filter((entry) => entry.status === "ready-for-apply").length} item(s) ready for apply; ${plan.filter((entry) => entry.status === "needs-secondary-approval").length} require secondary approval.`,
      { ready: plan.filter((entry) => entry.status === "ready-for-apply").length, secondaryApproval: plan.filter((entry) => entry.status === "needs-secondary-approval").length, rejected: plan.filter((entry) => entry.status === "rejected").length }
    ),
    writePerformed: false,
...
export function approvalResultReview(
  plan: Record<string, unknown>[] = [],
  results: Record<string, unknown>[] = [],
  currentItems: WorkItemSummary[] = []
): Report & { writePerformed: false; verification: Array<Record<string, unknown>>; auditEvents: Array<Record<string, unknown>> } {
  ...
  return {
    ...report(
      ...
      verification.filter((entry) => !entry.verified).map((entry) => finding(numberFrom(entry.workItemId), stringFrom(entry.title), 80, [stringFrom(entry.applyResult), stringFrom(entry.currentState)], stringFrom(entry.recommendation))),
      `${verification.filter((entry) => entry.verified).length}/${verification.length} approval result(s) verified.`,
      { verified: verification.filter((entry) => entry.verified).length, total: verification.length }
    ),
    writePerformed: false,
    verification,
    auditEvents
...
export function auditTrail(
  ...
  options: Record<string, unknown> = {}
): Report & { writePerformed: false; trail: Array<Record<string, unknown>> } {
  ...
  return {
    ...report(
      ...
      normalized.map((event) => finding(numberFrom(event.workItemId), `${event.action} by ${event.actor}`, event.action === "overridden" ? 80 : 45, [event.rationale], "Keep the rationale and verification outcome linked to the Work Item.")),
      `${normalized.length} decision event(s) normalized for audit review.`,
      { accepted: countActions(normalized, "accepted"), rejected: countActions(normalized, "rejected"), overridden: countActions(normalized, "overridden") }
    ),
    writePerformed: false,
    trail: normalized
  };
...
export function roleCockpitConfig(
  items: WorkItemSummary[],
  roleConfig: Record<string, unknown> = {},
  policy: Record<string, unknown> = {}
): Report & { writePerformed: false; cockpits: Array<Record<string, unknown>> } {
  ...
  return {
    ...report(
      ...
      cockpits.map((cockpit, index) => finding(undefined, stringFrom(cockpit.title), 50 + index, [`${arrayOfStrings(cockpit.defaultReports).length} default reports`], "Use this role cockpit as the default landing view for the team.")),
      `${cockpits.length} role-specific cockpit(s) prepared.`,
      { roles: cockpits.length, assessedItems: items.length }
    ),
    writePerformed: false,
    cockpits
  };
...
export function adminConsoleConfig(
  config: Record<string, unknown> = {}
): Report & { writePerformed: false; config: Record<string, unknown>; validation: Array<Record<string, unknown>> } {
  const normalized = {
  ...
  return {
    ...report(
      ...
      validation.filter((item) => !item.ready).map((item, index) => finding(undefined, stringFrom(item.name), 70 + index, [stringFrom(item.recommendation)], "Complete before production rollout.")),
      `${validation.filter((item) => item.ready).length}/${validation.length} production controls are ready.`,
      { controls: validation.length, ready: validation.filter((item) => item.ready).length }
    ),
    writePerformed: false,
    config: normalized,
    validation
...
export function automatedReminderPlan(
  items: WorkItemSummary[],
  options: Record<string, unknown> = {}
): Report & { writePerformed: false; reminders: Array<Record<string, unknown>> } {
  const watchlist = watchlistReport(items, options).findings;
  const benefit = (benefitRealizationFollowup(items as never, options).followups || []) as Record<string, unknown>[];
  ...
  return {
    ...report(
      ...
      reminders.map((reminder) => finding(numberFrom(reminder.workItemId), stringFrom(reminder.title), 60, [stringFrom(reminder.type), `next run ${reminder.nextRun}`], "Create a Codex automation or external scheduler from this no-write plan.")),
      `${reminders.length} reminder recommendation(s) prepared. This tool does not schedule or send.`,
      { reminders: reminders.length, cadenceDays }
    ),
    writePerformed: false,
    reminders
  };
...
export function decisionPackExport(
  ...
  evidence: Record<string, unknown>[] = [],
  options: Record<string, unknown> = {}
): Report & { writePerformed: false; pack: Record<string, unknown>; markdown: string; manifest: Record<string, unknown> } {
  ...
  const steering = aiSteeringCommitteePack(items as never, [evidenceReport], options);
  const auditPack = auditDecisionLog(items as never, evidence);
  const handover = handoverPackGenerator(items as never, evidence, options);
  const rhythm = operatingRhythmPlanner(items as never, options);
  const pack = {
  ...
  return {
    ...report(
      ...
      steering.findings.slice(0, 5),
      `Decision pack prepared for ${audience}.`,
      { items: items.length, evidence: evidence.length }
    ),
    writePerformed: false,
    pack,
    markdown,
...
export function decisionPackImport(
  artifact: Record<string, unknown> = {}
): Report & { writePerformed: false; imported: Record<string, unknown>; validation: Array<Record<string, unknown>> } {
  ...
  return {
    ...report(
      "Decision Pack Import Review",
      validation.filter((item) => !item.ready).map((item, index) => finding(undefined, stringFrom(item.name), 70 + index, [stringFrom(item.recommendation)], "Fix the Decision Pack artifact before importing.")),
      `${validation.filter((item) => item.ready).length}/${validation.length} Decision Pack import control(s) passed.`,
      { controls: validation.length, ready: validation.filter((item) => item.ready).length }
    ),
    writePerformed: false,
    imported,
...
function finding(id: number | undefined, title: string, score: number, signals: string[], recommendation: string): Finding {
  return { id, title, score, signals, recommendation };
}

function snapshotMetrics(items: WorkItemSummary[]): Record<string, number> {
...
function queueRisk(item: WorkItemSummary): string {
  if ((item.priority || 99) <= 1 || item.tags.some((tag) => /audit|compliance|security|finance/i.test(tag))) return "high";
  if (!item.assignedTo || daysSince(item.changedDate) > 21) return "medium";
  return "low";
}

function normalizeDecisionAction(value: string): string {
  ...
  if (normalized.includes("accept") || normalized.includes("approve")) return "accepted";
  return normalized || "recorded";
}

function roleTitle(role: string): string {
  if (role.includes("cio") || role.includes("executive")) return "CIO Portfolio Cockpit";
  if (role.includes("compliance")) return "Compliance Evidence Cockpit";
  if (role.includes("scrum") || role.includes("delivery")) return "Scrum Master Flow Cockpit";
  return "Product Owner Decision Cockpit";
}

function reportsForRole(role: string): string[] {
  if (role.includes("cio") || role.includes("executive")) return ["steering-pack", "portfolio-fitness", "enterprise-risk", "benefit-realization"];
  if (role.includes("compliance")) return ["evidence-ledger", "compliance-readiness", "policy-as-code", "audit-trail"];
  if (role.includes("scrum") || role.includes("delivery")) return ["watchlist", "flow-mining", "approval-queue", "operating-rhythm"];
  return ["requirement-decision", "gap-analysis", "approval-queue", "decision-memory"];
}

function decisionRightsForRole(role: string): string[] {
  if (role.includes("cio") || role.includes("executive")) return ["approve investment", "resolve escalation", "accept portfolio risk"];
  if (role.includes("compliance")) return ["approve evidence exception", "reject unsupported closure", "request audit pack"];
  if (role.includes("scrum") || role.includes("delivery")) return ["sequence cleanup", "raise blocker", "confirm delivery follow-up"];
  return ["prioritize", "accept requirement rewrite", "approve scope decision"];
}

function control(name: string, ready: boolean, recommendation: string): Record<string, unknown> {
  return { name, ready, status: ready ? "ready" : "missing", recommendation };
}

function countActions(events: Array<Record<string, unknown>>, action: string): number {
  return events.filter((event) => event.action === action).length;
}

function decisionPackManifest(pack: Record<string, unknown>, markdown: string): Record<string, unknown> {
...
function reminderSchedule(cadenceDays: number): Record<string, unknown> {
  return {
    cadenceDays,
