...
export interface ClosureGovernanceEntry {
  id: number;
  title: string;
  type: string;
  state: string;
  ...
  governanceStatus: "complete" | "review" | "gap";
}
...
export interface AuditDecisionEntry {
  ...
  title: string;
  decision: string;
  source: "comment" | "update" | "field" | "state";
  actor?: string;
  date?: string;
  rationale?: string;
  evidenceSignals: string[];
...
export interface HygienePreviewAction {
  id: number;
  title: string;
  actionType: "add-evidence-tag" | "request-owner" | "refresh-stale-item" | "complete-acceptance-criteria" | "review-terminal-child";
  rationale: string;
...
export function closureGovernanceLedger(
  items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>,
  updates: Record<string, unknown>[] = [],
  options: EvidenceLedgerOptions = {}
): Report & { writePerformed: false; ledger: ClosureGovernanceEntry[] } {
...
export function auditDecisionLog(
  items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>,
  evidence: Record<string, unknown>[] = [],
  options: EvidenceLedgerOptions = {}
): Report & { writePerformed: false; decisions: AuditDecisionEntry[] } {
  const normalized = normalizeItems(items);
  const byId = new Map(normalized.map((item) => [item.id, item]));
  const decisions: AuditDecisionEntry[] = [];
  ...
  for (const record of evidence) {
    ...
    if (!DECISION_PATTERN.test(text)) continue;
...
export function boardHygieneAutomationPreview(
  items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>,
  options: EvidenceLedgerOptions = {}
): Report & { writePerformed: false; approvalRequired: true; actions: HygienePreviewAction[] } {
...
export function evidencePackCompleteness(
  items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>,
  evidence: Record<string, unknown>[] = [],
  options: EvidenceLedgerOptions = {}
): Report & { writePerformed: false; packs: EvidencePackEntry[] } {
...
function closureEntry(item: NormalizedItem, updates: Record<string, unknown>[]): ClosureGovernanceEntry {
  ...
  if (!closedDate) missing.push("missing closure date");
  if (!closedBy) missing.push("missing closure actor");
...
function hygieneActions(item: NormalizedItem, allItems: NormalizedItem[], asOf: Date, staleDays: number, evidenceTag: string): HygienePreviewAction[] {
  const actions: HygienePreviewAction[] = [];
...
function actionFor(item: NormalizedItem, actionType: HygienePreviewAction["actionType"], rationale: string, patchPreview: Array<Record<string, unknown>>, risk: HygienePreviewAction["risk"]): HygienePreviewAction {
  return {
    id: item.id,
    title: item.title,
    actionType,
...
function evidencePackEntry(item: NormalizedItem, evidence: Record<string, unknown>[], requirements: string[]): EvidencePackEntry {
  const text = `${item.title} ${item.description} ${item.acceptanceCriteria} ${evidence.map(evidenceText).join(" ")}`;
  const present: string[] = [];
  const missing: string[] = [];
  for (const requirement of requirements) {
    if (requirementPresent(requirement, item, evidence, text)) present.push(requirement);
...
function requiredEvidence(options: EvidenceLedgerOptions): string[] {
...
function requirementPresent(requirement: string, item: NormalizedItem, evidence: Record<string, unknown>[], text: string): boolean {
  if (requirement.includes("owner") || requirement.includes("assigned")) return Boolean(item.assignedTo);
  if (requirement.includes("acceptance")) return Boolean(item.acceptanceCriteria);
  if (requirement.includes("closure") || requirement.includes("state")) return Boolean(item.state);
  if (requirement.includes("artifact") || requirement.includes("attachment") || requirement.includes("evidence")) return hasEvidence(item) || evidence.length > 0 || EVIDENCE_PATTERN.test(text);
  if (requirement.includes("decision") || requirement.includes("rationale")) return DECISION_PATTERN.test(text) || item.description.length >= 80;
  if (requirement.includes("priority")) return item.priority !== undefined;
  if (requirement.includes("link")) return item.links.length > 0;
  return text.toLowerCase().includes(requirement.toLowerCase());
}

function evidenceSignals(item: NormalizedItem, extraText = ""): string[] {
  ...
  if (item.tags.some((tag) => /evidence|audit|approved|validated|verified/i.test(tag))) signals.push(`evidence tag: ${item.tags.filter((tag) => /evidence|audit|approved|validated|verified/i.test(tag)).join(", ")}`);
  if (EVIDENCE_PATTERN.test(`${item.description} ${extraText}`)) signals.push("evidence keywords in text");
  return signals;
}

function hasEvidence(item: NormalizedItem): boolean {
  return evidenceSignals(item).length > 0;
}

function groupByWorkItemId(records: Record<string, unknown>[]): Map<number, Record<string, unknown>[]> {
...
function workItemId(record: Record<string, unknown>): number | undefined {
...
function sourceFromRecord(record: Record<string, unknown>): AuditDecisionEntry["source"] {
  const type = `${clean(record.type)} ${clean(record.source)}`.toLowerCase();
  if (type.includes("comment")) return "comment";
  if (type.includes("update") || record.revisedFields) return "update";
  return "field";
}

function dedupeDecisions(decisions: AuditDecisionEntry[]): AuditDecisionEntry[] {
  const seen = new Set<string>();
  const unique: AuditDecisionEntry[] = [];
...
function actorFrom(record: Record<string, unknown>): string | undefined {
...
function byStatusThenId(left: ClosureGovernanceEntry, right: ClosureGovernanceEntry): number {
  const rank = { gap: 0, review: 1, complete: 2 };
  return rank[left.governanceStatus] - rank[right.governanceStatus] || left.id - right.id;
}

function byDecisionDateThenId(left: AuditDecisionEntry, right: AuditDecisionEntry): number {
  return (left.date || "").localeCompare(right.date || "") || left.id - right.id || left.decision.localeCompare(right.decision);
}

function byActionPriority(left: HygienePreviewAction, right: HygienePreviewAction): number {
...
function byFindingPriority(left: Finding, right: Finding): number {
  return (right.score || 0) - (left.score || 0) || (left.id || 0) - (right.id || 0) || left.title.localeCompare(right.title);
}

function report(title: string, findings: Finding[], summary: string, options: EvidenceLedgerOptions): Report {
  return {
    title,
