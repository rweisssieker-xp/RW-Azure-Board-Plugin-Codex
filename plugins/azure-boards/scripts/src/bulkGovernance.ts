...
export interface BulkCloseTarget {
  id: number;
  title: string;
  type: string;
  currentState: string;
...
export function wsjfConsistencyCheck(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>): Report {
  const findings: Finding[] = normalizeItems(items)
    .map((item) => {
      ...
      if (values.costOfDelay !== undefined && values.businessValue !== undefined && values.timeCriticality !== undefined && values.riskReduction !== undefined) {
        ...
        if (values.costOfDelay !== sum) notes.push(`CostOfDelay ${values.costOfDelay} does not equal BV+TC+RR ${sum}`);
      }
      if (values.costOfDelay !== undefined && (values.jobDuration === undefined || values.jobDuration <= 0)) notes.push("CostOfDelay is present but JobDuration is missing or invalid");
      if (signals.includes("Compliance") && ((values.timeCriticality ?? 0) < 3 || (values.riskReduction ?? 0) < 3)) notes.push("compliance signal with low TimeCriticality/RiskReduction");
      if (signals.includes("Financial") && ((values.businessValue ?? 0) < 3 || (values.costOfDelay ?? 0) < 3)) notes.push("financial signal with low BusinessValue/CostOfDelay");
      if (signals.includes("Problem") && ((values.timeCriticality ?? 0) < 3 && (values.riskReduction ?? 0) < 3)) notes.push("problem/defect signal with low urgency and risk reduction");
      if (item.description.length < 140 && highWsjf(values)) notes.push("high WSJF values despite weak description");
...
export function businessValueEstimate(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>): Report & { estimates: Array<Record<string, unknown>> } {
...
export function attachmentEvidenceSummary(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>, attachmentTexts: Record<string, unknown>[] = []): Report {
  ...
  const findings = normalizeItems(items).map((item) => {
    ...
    if (attachments.length) signals.push(`files: ${attachments.slice(0, 5).map((attachment) => attachment.name).join(", ")}`);
    if (suppliedTexts.length) signals.push(`evidence: ${suppliedTexts.join(" ").slice(0, 240)}`);
    return {
      id: item.id,
      title: item.title,
...
export function closeCandidates(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>, options: Record<string, unknown> = {}): Report {
...
export function bulkClosePreview(
  ...
  const targets: BulkCloseTarget[] = [];
  const skipped: BulkClosePreview["skipped"] = [];
...
export function parentChildCleanup(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>): Report {
  const normalized = normalizeItems(items);
  const byId = new Map(normalized.map((item) => [item.id, item]));
  const findings: Finding[] = [];
  for (const item of normalized) {
    if (item.type.toLowerCase() !== "task" || isTerminal(item.state)) continue;
    const parent = item.parentId ? byId.get(item.parentId) : undefined;
    if (!parent || parent.type.toLowerCase() !== "requirement" || !isTerminal(parent.state)) continue;
    findings.push({
      id: item.id,
      title: item.title,
      score: 90,
...
function normalizeItems(items: Array<WorkItem | WorkItemSummary | Record<string, unknown>>): Array<{
  id: number;
  type: string;
  title: string;
  state: string;
  ...
  return items.map((raw) => {
    ...
    const summary = fields ? toSummary(raw as WorkItem) : (raw as WorkItemSummary);
    return {
...
function targetFor(item: ReturnType<typeof normalizeItems>[number], targetState: string, reason: string, childImpact: BulkCloseTarget[]): BulkCloseTarget {
...
function childTargets(parent: ReturnType<typeof normalizeItems>[number], allItems: ReturnType<typeof normalizeItems>, reason: string): BulkCloseTarget[] {
  return allItems
...
function businessSignals(item: ReturnType<typeof normalizeItems>[number]): string[] {
...
function euroEstimate(item: ReturnType<typeof normalizeItems>[number], signals: string[], wsjf?: number): { low: number; high: number; rationale: string } {
  let low = 5_000;
  let high = 20_000;
  const rationale: string[] = [];
  const title = item.title.toLowerCase();
  const set = (nextLow: number, nextHigh: number, reason: string) => {
    low = Math.max(low, nextLow);
    high = Math.max(high, nextHigh);
    rationale.push(reason);
  };
  if (/wup|schnittstelle|ssis|addone/.test(title) || signals.includes("Integration")) set(50_000, 180_000, "integration/stammdaten leverage");
  if (signals.includes("Compliance")) set(40_000, 150_000, "compliance or audit risk");
  if (signals.includes("Financial")) set(25_000, 120_000, "financial process impact");
  if (signals.includes("Production")) set(30_000, 160_000, "production/logistics productivity");
  if ((wsjf ?? 0) >= 100) {
...
function wsjfValues(raw: Record<string, unknown>): {
...
function wsjfSummary(values: ReturnType<typeof wsjfValues>): string {
...
function highWsjf(values: ReturnType<typeof wsjfValues>): boolean {
  return Math.max(values.businessValue ?? 0, values.timeCriticality ?? 0, values.riskReduction ?? 0, values.costOfDelay ?? 0) >= 50;
}

function attachmentRelations(raw: Record<string, unknown>): Array<{ name: string; url?: string }> {
...
function addSignal(signals: string[], text: string, pattern: RegExp, signal: string): void {
...
function report(title: string, findings: Finding[], summary: string): Report {
  return {
    title,
    generatedAt: new Date().toISOString(),
    summary,
