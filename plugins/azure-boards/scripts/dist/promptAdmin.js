...
export function runPrompt(input) {
    const name = requiredString(input.name, "name");
    const artifact = loadNamedArtifact(PROMPT_KIND, name);
    if (!artifact)
...
export function getAdminConfig() {
    const artifact = loadNamedArtifact(ADMIN_KIND, "default");
    return artifact || {
        kind: ADMIN_KIND,
        name: "default",
        savedAt: "",
...
export function validateAdminConfig(input) {
    const config = input.config && typeof input.config === "object" ? input.config : input;
    const issues = [];
    const models = recordArray(config.models);
    if (!models.length)
        issues.push({ severity: "medium", message: "No model ranking configured; deterministic local fallback will be used." });
