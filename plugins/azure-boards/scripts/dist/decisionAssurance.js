...
export function decisionMemory(workItems, decisions = [], outcomes = []) {
...
export function recommendationQualityScore(recommendations = [], outcomes = []) {
...
export function valueInflationDetector(workItems, options = {}) {
...
export function decisionCourt(workItems, recommendations = []) {
    ...
    const findings = cases.filter((entry) => entry.missingFacts.length || entry.contra.length > 1).map((entry) => finding(Number(entry.id), String(entry.title), 100 - Number(entry.confidence), [`action ${entry.action}`, `missing ${entry.missingFacts.join(", ") || "none"}`], "Review court arguments before approving the recommendation."));
...
export function requirementContractLifecycle(workItems, options = {}) {
...
export function scenarioWarRoom(workItems, scenarios = []) {
    const items = normalizeItems(workItems);
    ...
    const findings = rows.map((row) => finding(undefined, String(row.name), 100 - Number(row.confidence), [`at risk ${row.atRisk.length}`, `protected ${row.protectedItems.length}`], String(row.recommendation)));
...
export function autonomousGovernanceAgent(workItems, evidence = [], options = {}) {
    ...
    const actionPreviews = watchlist.slice(0, 20).map((entry) => ({ id: entry.id, title: entry.title, patchPreview: [{ op: "add", path: "/fields/System.Tags", value: "Governance Review" }], rationale: `Governance watchlist: ${entry.reason.join(", ")}`, writePerformed: false }));
    const findings = watchlist.map((entry) => finding(Number(entry.id), String(entry.title), 75, entry.reason, "Review in the next governance cycle and approve any patch separately."));
...
function defaultDecision(item) {
    ...
    if (item.description.length < 80 || item.acceptanceCriteria.length < 40)
        return "defer pending evidence";
    return "continue with controls";
}
function proArguments(item, action) {
    const args = [];
    if (/close|defer/i.test(action) && item.description.length < 80)
        args.push("weak decision-grade description");
    if (/continue|build|keep/i.test(action) && item.acceptanceCriteria.length >= 40)
        args.push("acceptance criteria present");
    if (item.assignedTo)
        args.push("accountable owner present");
    if (item.tags.some((tag) => /evidence|audit|approved/i.test(tag)))
        args.push("evidence tag present");
    return args.length ? args : ["recommendation has plausible board signals"];
}
function contraArguments(item, action) {
    const args = [];
    if (/close|defer/i.test(action) && item.acceptanceCriteria.length >= 40)
        args.push("acceptance criteria may indicate real delivery scope");
    if (/continue|build|keep/i.test(action) && item.description.length < 80)
        args.push("business problem is weakly documented");
    if (!item.assignedTo)
        args.push("missing accountable owner");
    return args;
}
function missingFactsFor(item) {
...
function metricFor(item) {
    ...
    if (/support|ticket|customer/.test(text))
        return "support ticket reduction or customer self-service rate";
    if (/finance|invoice|billing|cost/.test(text))
        return "financial cycle time, cost reduction, or invoice accuracy";
    if (/regulatory|audit|compliance/.test(text))
        return "audit finding count or compliance evidence completeness";
    if (/production|warehouse|inventory/.test(text))
        return "process throughput, error rate, or inventory accuracy";
    return "measurable business process outcome";
}
function scenarioRisk(item, scenario) {
    const text = `${item.title} ${item.description} ${item.tags.join(" ")}`.toLowerCase();
    if (numberFrom(scenario.budgetFactor) !== undefined && Number(scenario.budgetFactor) < 1 && !/compliance|regulatory|finance|customer/.test(text))
        return true;
    if (scenario.goLiveFixed === true && /block|delay|risk|dependency/.test(text))
        return true;
    if (scenario.complianceFirst === true && !/compliance|regulatory|audit|security/.test(text) && !item.assignedTo)
        return true;
    return false;
}
function qualityStatus(action, outcome) {
    if (!outcome)
        return "unknown";
    const status = `${stringFrom(outcome.status)} ${stringFrom(outcome.decision)} ${stringFrom(outcome.outcome)}`.toLowerCase();
    if (/reversed|reopened|failed|wrong/.test(status))
        return "reversed";
    if (/stale|no evidence|missing/.test(status))
        return "stale";
    if (/confirmed|accepted|successful|realized|closed/.test(status))
        return "confirmed";
    return /close|defer/i.test(action) && /built|implemented/i.test(status) ? "reversed" : "unknown";
}
function learningFor(status) {
    if (status === "confirmed")
        return "Keep similar scoring signals.";
    if (status === "reversed")
        return "Reduce confidence for similar recommendations until stronger evidence exists.";
    if (status === "stale")
        return "Require outcome evidence before considering the recommendation validated.";
