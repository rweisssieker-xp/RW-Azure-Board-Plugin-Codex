...
export { attachmentEvidenceSummary, bulkClosePreview, businessValueEstimate, closeCandidates, parentChildCleanup, wsjfConsistencyCheck } from "./bulkGovernance.js";
export { deliverySystemCorrelation } from "./correlationAnalytics.js";
export { autonomousGovernanceAgent, decisionCourt, decisionMemory, recommendationQualityScore, requirementContractLifecycle, scenarioWarRoom, valueInflationDetector } from "./decisionAssurance.js";
...
export { adoptionCockpit, businessDigitalTwin, enterpriseRiskHeatmap, eventLogProcessMining, externalEvidenceImport, modelRiskGovernance, policyStudio, promptEvalSuite, roiConfidenceWorkflow, stakeholderInfluenceMap } from "./enterpriseValueTrust.js";
export { adoptionExperimentDesigner, autonomousFollowupScheduler, connectorReadinessAudit, customerValueCaseBuilder, evidenceIngestionPipeline, licensePackagingAdvisor, marketplaceSubmissionReadiness, orgRolloutReadiness, proprietarySignalCatalog, securityPrivacyReview } from "./productizationMoat.js";
export { adminConsoleConfig, approvalApplyPlan, approvalQueue, approvalResultReview, auditTrail, automatedReminderPlan, createPersistentBaseline, createPersistentSnapshot, decisionPackExport, decisionPackImport, roleCockpitConfig } from "./productOperatingSystem.js";
export { aiExceptionRegister, autonomousBoardAuditor, benefitRealizationFollowup, cleanupCampaignManager, decisionMeetingCopilot, dependencyBlockerGraph, financialBacklogLedger, handoverPackGenerator, okrAlignmentScorer, operatingRhythmPlanner, portfolioFitnessIndex, complianceReadinessReview, migrationCutoverReadiness, processOwnerControlTower, requirementConfidenceScore, requirementRewriteStudio } from "./governanceOperatingSystem.js";
...
export { deletePrompt, getAdminConfig, listPrompts, runPrompt, savePrompt, validateAdminConfig } from "./promptAdmin.js";
export { convertRequirement, elicitRequirements, requirementGapAnalysis, transformWorkItemText } from "./requirementsWorkbench.js";
export { applyTestCasePlan, generateRegressionSuite, generateTestCases, generateUatSuite } from "./testFactory.js";
export { applyTraceabilityPlan, defectTraceability, requirementTestTraceability, testCoverageAnalysis } from "./traceabilityEngine.js";
export { generateDiagram, generateMockup, generateSopDocument } from "./visualDocumentFactory.js";
export { validatePolicyPack } from "./policyPack.js";
export { benefitRealizationTracking, costAvoidanceAnalysis, costAvoidanceByClosure, erpDomainImpactScoring, portfolioRationalization } from "./portfolioEngine.js";
export { redactSecrets, safeErrorMessage } from "./security.js";
export { aiBusinessCaseGenerator, aiSteeringCommitteePack, automatedBoardDueDiligenceReport, changePortfolioSimulator, decisionTraceabilityGraph, erpProcessCriticalityModel, outcomeRealizationCockpit, policyAsCodeEvaluation, requirementInvestDivestMatrix, valueLeakageDetector } from "./steeringEngine.js";
import { briefExport as baseBriefExport } from "./briefExport.js";
import { createProcessBaseline as baseCreateProcessBaseline, processDriftDetection as baseProcessDriftDetection } from "./baselineAnalytics.js";
import { actionPlan as baseActionPlan, watchlistReport as baseWatchlistReport } from "./operationalAnalytics.js";
import { capacityForecast as baseCapacityForecast, costOfDelayRadar as baseCostOfDelayRadar, processSimulator as baseProcessSimulator } from "./strategicAnalytics.js";
...
const QUALITY_FIELDS = ["title", "description", "acceptanceCriteria"];
export function deliveryRiskRadar(items) {
...
export function milestoneForecast(items) {
    ...
    return report("Milestone Confidence Forecast", [
        {
...
export function scopeCreepDetector(current, previous = []) {
    ...
    const findings = [
        ...newItems.slice(0, 10).map((item) => findingForItem(item, highPriorityNew.includes(item) ? 80 : 45, ["new since comparison snapshot"], "Confirm whether this is approved scope or backlog noise."))
    ];
...
export function statusBrief(items) {
    const open = items.filter((item) => !isClosed(item));
...
export function decisionDebt(items) {
    const findings = items
...
export function bottleneckMining(items) {
...
export function workflowConformance(items, policy = {}) {
    ...
    for (const item of items) {
        ...
        if (!item.title || item.title.length < 8)
            signals.push("title is missing or too short");
        if (!item.assignedTo && !isClosed(item))
            signals.push("open item has no assignee");
        for (const tag of requiredTags) {
            if (!item.tags.map((t) => t.toLowerCase()).includes(tag.toLowerCase()))
                signals.push(`missing required tag ${tag}`);
        }
        if (requiredTypes.length && !requiredTypes.includes(item.type))
            signals.push(`type ${item.type} is outside allowed policy`);
        if (signals.length) {
...
export function slaAgingMonitor(items, slaDays = 14) {
    const findings = items
        ...
        const signals = [];
        if (age > slaDays)
            signals.push(`age ${age} days exceeds SLA ${slaDays}`);
        if (stale > Math.ceil(slaDays / 2))
            signals.push(`no update for ${stale} days`);
...
export function rootCausePatterns(items) {
    const patterns = [
...
export function processRecommendations(items) {
    ...
    const findings = [
        ...
        },
        {
            ...
            severity: conformance > 8 ? "high" : "medium",
...
export function governanceScore(items, policy = {}) {
    ...
    return report("Process Governance Copilot", [
        {
...
export function policyGapDetector(items, policy = {}) {
    return {
...
export function changeImpact(items, proposedRule) {
    const rule = proposedRule.toLowerCase();
    const impacted = items.filter((item) => {
        if (rule.includes("assignee"))
            return !item.assignedTo;
        if (rule.includes("tag"))
            return item.tags.length === 0;
        if (rule.includes("priority"))
            return item.priority === undefined;
        if (rule.includes("closed"))
            return isClosed(item);
        return !isClosed(item);
    });
    return report("Change Impact Analyzer", impacted.slice(0, 25).map((item) => findingForItem(item, 60, [`would be affected by rule: ${proposedRule}`], "Review before enforcing the proposed process change.")), { summary: `${impacted.length} Work Items would likely be affected by the proposed rule.` });
}
export function crossTeamBenchmark(items) {
...
export function auditEvidencePack(items, policy = {}) {
...
export function improveWorkItem(input) {
    ...
    if (!title || title.length < 12)
        suggestions.title = `${type}: clarify user, outcome, and constraint`;
    if (!description || description.length < 40)
        suggestions.description = "Problem:\n\nUser impact:\n\nCurrent behavior:\n\nExpected behavior:\n\nNotes:";
...
export function naturalLanguageToWiql(input) {
    ...
    if (text.includes("bug"))
        clauses.push("[System.WorkItemType] = 'Bug'");
    if (text.includes("task"))
        clauses.push("[System.WorkItemType] = 'Task'");
    if (text.includes("open") || text.includes("offen"))
        clauses.push("[System.State] <> 'Closed'");
    if (text.includes("unassigned") || text.includes("nicht zugewiesen"))
        clauses.push("[System.AssignedTo] = ''");
    if (text.includes("high") || text.includes("hoch"))
        clauses.push("[Microsoft.VSTS.Common.Priority] <= 2");
    if (text.includes("stale") || text.includes("alt"))
        clauses.push("[System.ChangedDate] < @Today - 7");
    return {
...
export function actionPlan(items, policy = {}, options = {}) {
...
export function createProcessBaseline(items, updatesOrPolicy = {}, policy = {}) {
...
export function costOfDelayRadar(items, options = {}) {
    return { ...baseCostOfDelayRadar(items, options), writePerformed: false };
}
export function processSimulator(items, scenario) {
    return { ...baseProcessSimulator(items, scenario), writePerformed: false };
}
export function capacityForecast(items, options = {}) {
    return { ...baseCapacityForecast(items, options), writePerformed: false };
}
export function briefExport(reportOrReports, options = {}) {
...
export function projectCockpit(items, policy = {}) {
    const open = items.filter((item) => !isClosed(item));
    ...
    const riskScore = Math.min(100, Math.round((stale.length / Math.max(open.length, 1)) * 30 +
...
export function flowMiningFromUpdates(items, updates = []) {
    ...
    const findings = Object.entries(transitionCounts)
...
export function commentIntelligence(items, comments = []) {
    const itemById = new Map(items.map((item) => [item.id, item]));
    ...
    for (const [id, itemComments] of Object.entries(byItem)) {
        ...
        if (includesAny(combined, BLOCKER_WORDS))
            signals.push("blocker/dependency discussion");
        if (includesAny(combined, DECISION_WORDS))
            signals.push("decision/approval discussion");
        if (includesAny(combined, RISK_WORDS))
            signals.push("risk/escalation discussion");
        if (includesAny(combined, CUSTOMER_WORDS))
            signals.push("customer/production impact discussion");
        if (signals.length >= 2) {
            const item = itemById.get(Number(id));
            findings.push({
                id: Number(id),
...
export function roleBasedReport(items, roleOrOptions = "executive", policy = {}) {
    const role = typeof roleOrOptions === "string" ? roleOrOptions.toLowerCase() : String(roleOrOptions.role || "executive").toLowerCase();
    const effectivePolicy = typeof roleOrOptions === "string" ? policy : (roleOrOptions.policy && typeof roleOrOptions.policy === "object" ? roleOrOptions.policy : policy);
    const source = role.includes("exec") || role.includes("leadership")
...
export function policyPackSummary(items, policyPack = {}) {
...
function report(title, findings, options = {}) {
    return {
        title,
...
function findingForItem(item, score, signals, recommendation) {
    return {
        id: item.id,
...
function arrayPolicy(value) {
    return Array.isArray(value) ? value.filter((item) => typeof item === "string") : [];
}
function numericPolicy(value) {
...
function normalizeUpdate(update) {
    ...
    const stateField = fields["System.State"] && typeof fields["System.State"] === "object" ? fields["System.State"] : {};
    const workItem = update.workItem && typeof update.workItem === "object" ? update.workItem : {};
    return {
...
function normalizeComment(comment) {
    const nested = comment.comment && typeof comment.comment === "object" ? comment.comment : {};
    const workItem = comment.workItem && typeof comment.workItem === "object" ? comment.workItem : {};
    return {
...
function extractPolicies(policyPack) {
    const rawPolicies = policyPack.policies;
    if (Array.isArray(rawPolicies)) {
        return rawPolicies
            .filter((policy) => policy !== null && typeof policy === "object" && !Array.isArray(policy))
            .map((policy, index) => ({
            name: stringFrom(policy.name) || stringFrom(policy.title) || `Policy ${index + 1}`,
            rules: policy.rules && typeof policy.rules === "object" && !Array.isArray(policy.rules) ? policy.rules : policy
        }));
    }
    if (rawPolicies && typeof rawPolicies === "object") {
        return Object.entries(rawPolicies)
            .filter((entry) => entry[1] !== null && typeof entry[1] === "object" && !Array.isArray(entry[1]))
            .map(([name, rules]) => ({ name, rules }));
    }
    return [{ name: stringFrom(policyPack.name) || "Default Policy", rules: policyPack }];
}
function roleRecommendation(role, recommendation) {
    if (role.includes("exec") || role.includes("leadership"))
        return `Decide ownership, escalation, or scope tradeoff. ${recommendation}`;
    if (role.includes("qa") || role.includes("quality") || role.includes("test"))
        return `Convert this into verifiable evidence or a quality gate. ${recommendation}`;
    if (role.includes("product") || role.includes("po"))
        return `Clarify priority, customer value, and scope decision. ${recommendation}`;
    if (role.includes("scrum") || role.includes("delivery"))
        return `Turn this into a dated delivery impediment action. ${recommendation}`;
    return recommendation;
}
function roleNextActions(role) {
    if (role.includes("exec") || role.includes("leadership"))
        return ["Resolve top ownership/escalation decisions.", "Confirm milestone confidence and scope tradeoffs."];
    if (role.includes("qa") || role.includes("quality") || role.includes("test"))
        return ["Attach missing evidence for policy and SLA findings.", "Prioritize defects or reopened items with repeated risk signals."];
    if (role.includes("product") || role.includes("po"))
        return ["Approve or reject new scope.", "Clarify acceptance criteria for high-risk items."];
    if (role.includes("scrum") || role.includes("delivery"))
        return ["Unblock stale work.", "Update owners and next actions before the next standup."];
...
function uniqueCount(values) {
    return new Set(values.filter((value) => typeof value === "number")).size;
}
