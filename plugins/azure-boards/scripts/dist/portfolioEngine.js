...
const VALUE_FIELDS = ["Custom.BusinessValue", "Microsoft.VSTS.Common.BusinessValue", "BusinessValue", "Business Value"];
const TARGET_BENEFIT_FIELDS = ["Custom.TargetBenefit", "Custom.ExpectedBenefit", "Custom.AnnualBenefit", "Custom.Benefit", "Business.TargetBenefit"];
const REALIZED_BENEFIT_FIELDS = ["Custom.RealizedBenefit", "Custom.ActualBenefit", "Custom.BenefitRealized", "Business.RealizedBenefit"];
const COST_FIELDS = ["Custom.Cost", "Custom.EstimatedCost", "Custom.Budget", "Microsoft.VSTS.Scheduling.OriginalEstimate", "Microsoft.VSTS.Scheduling.RemainingWork"];
const EFFORT_FIELDS = ["Custom.JobDuration", "Custom.Effort", "Microsoft.VSTS.Scheduling.Effort", "Microsoft.VSTS.Scheduling.StoryPoints", "Microsoft.VSTS.Scheduling.Size"];
const ERP_DOMAINS = [
...
export function portfolioRationalization(items, options = {}) {
    ...
    const findings = normalized
        ...
        if (domain)
            signals.push(`${domain.domain} impact ${domain.score}`);
        if (duplicateIds.length)
            signals.push(`possible duplicate/merge with ${duplicateIds.map((id) => `#${id}`).join(", ")}`);
...
export function benefitRealizationTracking(items, options = {}) {
    const normalized = normalizeItems(items);
    const asOf = referenceDate(normalized, options.asOfDate);
...
export function costAvoidanceByClosure(items, options = {}) {
    const normalized = normalizeItems(items);
    const asOf = referenceDate(normalized, options.asOfDate);
...
export function erpDomainImpactScoring(items, options = {}) {
    const normalized = normalizeItems(items);
    const asOf = referenceDate(normalized, options.asOfDate);
    const findings = normalized
        .map((item) => {
        ...
        if (domains.length > 1)
            signals.push(`cross-domain: ${domains.slice(0, 4).map((domain) => domain.domain).join(", ")}`);
        ...
        .sort(byScoreDesc)
        .slice(0, maxItems(options.maxFindings, 20));
    const domainCounts = ERP_DOMAINS.reduce((counts, domain) => {
...
function evidenceScore(item) {
    ...
    if (item.description.length >= 400)
        score += 15;
    if (firstNumber(item, TARGET_BENEFIT_FIELDS) !== undefined)
        score += 25;
    if (firstNumber(item, VALUE_FIELDS) !== undefined)
        score += 15;
    if (item.tags.length)
        score += 10;
    if (item.areaPath)
...
function rawEffort(item) {
    return firstNumber(item, EFFORT_FIELDS);
}
function rationalizationDecision(value, evidence, effort, stale, staleDays, duplicateCount, highValueThreshold, lowValueThreshold) {
    if (duplicateCount > 0)
        return "merge";
    if (value >= highValueThreshold && evidence >= 35)
        return "keep";
    if (value <= lowValueThreshold && stale >= staleDays)
        return "kill";
    if (value >= highValueThreshold && (evidence < 35 || effort > 75))
        return "rework";
    if (evidence < 25 || effort > value + 30)
        return "rework";
    return value >= 45 ? "keep" : "kill";
}
function rationalizationScore(decision, value, evidence, effort, stale, staleDays, duplicates) {
    if (decision === "keep")
        return Math.round(clamp(value * 0.7 + evidence * 0.3, 0, 100));
    if (decision === "merge")
        return Math.round(clamp(65 + duplicates * 15 + Math.min(20, value / 5), 0, 100));
    if (decision === "kill")
        return Math.round(clamp(45 + Math.max(0, stale - staleDays) / 3 + Math.max(0, 40 - value), 0, 100));
    return Math.round(clamp(40 + Math.max(0, 60 - evidence) + Math.max(0, effort - value) * 0.5, 0, 100));
}
function benefitEntry(item, asOf, realizationLagDays, minimumTargetBenefit) {
    const targetBenefit = firstNumber(item, TARGET_BENEFIT_FIELDS) ?? inferredBenefit(item);
    if (targetBenefit < minimumTargetBenefit)
        return undefined;
    ...
    if (missingAfterLag)
        signals.push(`missing realized benefit after ${realizationLagDays}-day lag`);
    return {
        item,
        score,
        targetBenefit,
...
function domainScores(item) {
    const text = textFor(item);
    return ERP_DOMAINS
        .map((domain) => {
        if (!domain.keywords.test(text))
            return undefined;
...
function recommendationForDecision(decision) {
    if (decision === "keep")
        return "Keep in the portfolio, confirm owner, and protect capacity if evidence remains current.";
    if (decision === "kill")
        return "Review for closure or parking; avoid further spend unless a stronger business case is added.";
    if (decision === "merge")
        return "Consolidate duplicate demand under one accountable parent before prioritization.";
    return "Rework the business case, benefit evidence, effort estimate, or scope before a keep/kill decision.";
}
function report(title, findings, asOf, options = {}) {
    return {
        title,
...
function typeCostMultiplier(item) {
...
function numberFromSignal(signals, prefix) {
...
function maxItems(value, fallback) {
