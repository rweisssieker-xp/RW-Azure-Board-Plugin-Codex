...
import { adminConsoleConfig, aiReadinessPromptGovernance, approvalApplyPlan, approvalQueue, approvalResultReview, auditTrail, approvalSimulation, automatedReminderPlan, autonomousGovernanceOperatingPlan, boardToValueMapping, createPersistentBaseline, createPersistentSnapshot, complianceEvidenceScore, decisionKnowledgeGraph, decisionMemoryLearning, decisionPackExport, decisionPackImport, executiveSteeringRoom, outcomeProofEngine, roleCockpitConfig, scopeCreepRadar } from "./productOperatingSystem.js";
...
const tools = [
    ...
    },
    {
        ...
        handler: async (args) => azure.getWorkItemComments(requiredString(args.organization, "organization"), requiredString(args.project, "project"), requiredNumber(args.id, "id"))
    },
    {
        name: "azure_boards_get_work_item_updates",
        description: "Fetch Work Item update history for flow mining and process analysis.",
        inputSchema: workItemEvidenceSchema(),
        handler: async (args) => azure.getWorkItemUpdates(requiredString(args.organization, "organization"), requiredString(args.project, "project"), requiredNumber(args.id, "id"))
    },
    {
        ...
        inputSchema: workItemEvidenceSchema(),
        handler: async (args) => azure.getWorkItemRevisions(requiredString(args.organization, "organization"), requiredString(args.project, "project"), requiredNumber(args.id, "id"))
    },
    {
        ...
        handler: async (args) => azure.listSavedQueries(requiredString(args.organization, "organization"), requiredString(args.project, "project"), Number(args.depth ?? 2))
    },
    {
        ...
        handler: async (args) => azure.listPullRequests(requiredString(args.organization, "organization"), requiredString(args.project, "project"), optionalString(args.repositoryId))
    },
    {
        ...
        handler: async (args) => azure.listBuilds(requiredString(args.organization, "organization"), requiredString(args.project, "project"), Number(args.top ?? 50))
    },
    {
    ...
    },
    {
        ...
        handler: (args) => processDriftDetection(readItems(args), objectArg(args.baseline), objectArg(args.policy))
    },
    {
    ...
    },
    {
        ...
        handler: (args) => briefExport((Array.isArray(args.reports) ? args.reports : objectArg(args.report)), objectArg(args.options))
    },
    {
        ...
        handler: (args) => synthesizeReport((Array.isArray(args.reports) ? args.reports : objectArg(args.report)), objectArg(args.options))
    },
    {
    ...
    },
    {
        ...
        handler: (args) => planApprovedActions(objectArg(args.actionPlanReport), objectArg(args.selection))
    },
    {
        ...
        handler: (args) => summarizeApplyResults(recordArrayArg(args.results, "results"))
    },
    {
...
function workItemEvidenceSchema() {
    return {
        type: "object",
        properties: {
            organization: { type: "string" },
...
async function applyBulkClosePlan(args) {
    ...
    const preview = objectArg(args.preview);
...
function flattenBulkCloseTargets(targets) {
    const flattened = [];
    const seen = new Set();
    for (const target of targets) {
...
function recordArrayArg(value, name) {
