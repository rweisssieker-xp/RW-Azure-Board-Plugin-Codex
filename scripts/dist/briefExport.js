...
function normalizeReports(reportOrReports) {
    ...
    return reports.map((report, index) => {
        ...
        return {
            ...
            nextActions: Array.isArray(report.nextActions) ? report.nextActions.filter((action) => typeof action === "string") : undefined
        };
    });
}
function normalizeType(type, reports) {
    const value = clean(type).toLowerCase().replace(/[_\s]+/g, "-");
    if (value === "weekly" || value === "weeklysteering" || value === "weekly-steering")
        return "weekly-steering";
    if (value === "daily" || value === "dailyrisk" || value === "daily-risk")
        return "daily-risk";
    if (value === "audit" || value === "auditupdate" || value === "audit-update")
        return "audit-update";
    const titles = reports.map((report) => report.title.toLowerCase()).join(" ");
    if (titles.includes("audit") || titles.includes("governance") || titles.includes("policy"))
        return "audit-update";
    if (titles.includes("risk") || titles.includes("sla") || titles.includes("aging"))
        return "daily-risk";
    return "weekly-steering";
}
function selectFindings(reports, profile) {
    return reports
...
function formatFindingLead(finding) {
...
function defaultActions(profile) {
...
function renderHtml(markdown) {
    const body = markdown
        ...
        if (line.startsWith("  - "))
            return `<li class="nested">${escapeHtml(line.slice(4))}</li>`;
        if (!line.trim())
            return "";
