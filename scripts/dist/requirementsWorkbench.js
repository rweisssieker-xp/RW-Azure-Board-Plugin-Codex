...
export function elicitRequirements(input) {
    const seeds = [
...
export function requirementGapAnalysis(workItems, evidence = []) {
    ...
    const findings = gaps.filter((entry) => entry.missing.length).map((entry) => finding(Number(entry.id), String(entry.title), 100 - Number(entry.qualityScore), entry.missing, String(entry.recommendation)));
...
export function transformWorkItemText(input) {
...
export function convertRequirement(input) {
    const item = normalizeItem(input.workItem && typeof input.workItem === "object" ? input.workItem : input, 0);
...
export function report(title, findings, summary, metrics = {}) {
...
export function finding(id, title, score, signals, recommendation) {
...
function transformText(text, operation, language) {
    const base = text || "No source text supplied.";
    if (/elaborate/i.test(operation))
        return `${base}\n\nDetails to clarify: business problem, target outcome, evidence, exception handling, owner, and acceptance criteria.`;
    if (/paraphrase/i.test(operation))
        return `Rephrased (${language}): ${firstSentence(base)} The requirement should state outcome, scope, and evidence clearly.`;
    if (/translate/i.test(operation))
        return `Translation draft (${language}): ${base}`;
    return firstSentence(base).slice(0, 500);
}
function convertContent(item, target) {
    if (/gherkin/i.test(target))
...
function evidenceSignals(item, evidence) {
    const signals = [];
    ...
    if (evidence.some((entry) => (numberFrom(entry.workItemId) ?? numberFrom(entry.id)) === item.id))
        signals.push("supplied evidence");
    return signals;
}
