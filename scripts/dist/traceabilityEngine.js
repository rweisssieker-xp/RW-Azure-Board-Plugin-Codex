...
export function requirementTestTraceability(workItems, testCases = []) {
    ...
    const tests = normalizeItems(testCases).filter((item) => /test/i.test(item.type) || /test/i.test(item.title));
...
export function testCoverageAnalysis(workItems, testCases = []) {
...
export function defectTraceability(workItems, defects = [], testResults = []) {
    const requirements = normalizeItems(workItems);
    const bugs = normalizeItems(defects).filter((item) => /bug|defect/i.test(item.type) || /bug|defect|failure/i.test(item.title));
...
export async function applyTraceabilityPlan(args, azure) {
