...
export function costOfDelayRadar(items, options = {}) {
    ...
    const findings = scored.map((entry) => findingForItem(entry.item, entry.score, [...entry.signals, `estimated ${horizonDays}-day delay cost ${Math.round(entry.delayCost)}`], entry.score >= 80
...
function scoreCostOfDelay(item, asOf, horizonDays, options) {
    ...
    if (customerMultiplier > 1)
        signals.push("customer/revenue impact language");
    if (blockerMultiplier > 1)
        signals.push("blocker/dependency language");
    if (riskMultiplier > 1)
        signals.push("risk/escalation language");
...
function recentThroughput(items, asOf, lookbackDays) {
...
function weightedSize(item) {
    ...
    if (type.includes("epic"))
        ...
        size += 3;
    else if (type.includes("story") || type.includes("requirement") || type.includes("pbi"))
        size += 1;
    if (hasAny(item, DEFECT_WORDS))
        size += 0.5;
...
function scoreFromDelta(deltaDays, wipPressure) {
    return Math.round(clamp(50 + deltaDays * 3 + wipPressure * 5, 0, 100));
}
function severityFromDelta(deltaDays, wipPressure) {
    if (deltaDays > 10 || wipPressure > 5)
        return "high";
    if (deltaDays > 0 || wipPressure > 0)
        return "medium";
    return "low";
}
function byScoredItem(a, b) {
