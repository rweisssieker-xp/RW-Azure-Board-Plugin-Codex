...
2. Use deployment placeholders such as `TODO_DEPLOY_HOSTED_MCP_URL` or `TODO_ENTRA_CLIENT_ID` only in pre-submission artifacts when the real hosted MCP endpoint or Microsoft Entra client id is not known.
...
3. Prefer explainable AI tools for process and delivery insights, especially:
   ...
   - `azure_boards_product_onboarding_wizard`
   - `azure_boards_product_policy_pack_catalog`
...
16. For WSJF and business-value reviews, compare `Custom.BusinessValue`, `Custom.TimeCriticality`, `Custom.RiskReduction`, `Custom.CostOfDelay`, and `Custom.JobDuration` against Description, attachments, status, priority, children, and aging. Treat USD-denominated values as conservative estimates and state assumptions.
...
19. For portfolio economics, use `azure_boards_ai_portfolio_rationalization`, `azure_boards_ai_benefit_realization_tracking`, `azure_boards_ai_cost_avoidance_by_closure`, and `azure_boards_ai_erp_domain_impact_scoring`. State assumptions for USD-denominated cost, benefit, and domain-impact estimates.
...
28. For product operating system work, use `azure_boards_product_snapshot_save`, `azure_boards_product_baseline_save`, `azure_boards_product_approval_queue`, `azure_boards_product_approval_apply_plan`, `azure_boards_product_approval_result_review`, `azure_boards_product_audit_trail`, `azure_boards_product_role_cockpits`, `azure_boards_product_admin_console`, `azure_boards_product_reminder_plan`, `azure_boards_product_decision_pack_export`, `azure_boards_product_decision_pack_import`, `azure_boards_product_outcome_proof_engine`, `azure_boards_product_decision_memory_learning`, `azure_boards_product_board_to_value_mapping`, `azure_boards_product_autonomous_governance_plan`, `azure_boards_product_compliance_evidence_score`, `azure_boards_product_scope_creep_radar`, `azure_boards_product_approval_simulation`, `azure_boards_product_executive_steering_room`, `azure_boards_product_decision_knowledge_graph`, `azure_boards_product_ai_readiness_prompt_governance`, `azure_boards_product_onboarding_wizard`, and `azure_boards_product_policy_pack_catalog`. Treat snapshots, baselines, approval queues, apply plans, result reviews, audit trails, role cockpit configs, reminders, decision packs, outcome proof, learning loops, value maps, governance plans, compliance scores, scope creep radar, approval simulations, executive steering rooms, decision graphs, AI readiness governance, onboarding plans, and policy pack catalogs as local product artifacts or no-write previews until the user explicitly approves a concrete Azure DevOps write.
